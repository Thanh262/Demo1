    ...
    (record, search, url) => {
        ...
        const beforeLoad = (scriptContext) => {
            ...
            if (tgType === scriptContext.UserEventType.VIEW) {
                ...
                addButtonToSalesOrder(form, scriptContext.newRecord);
            }
        }

        const addButtonToSalesOrder = (form, newRecord) => {
            let createUrl = url.resolveRecord({
                recordType : record.Type.SALES_ORDER,
                isEditMode : true,
                params : {
                    cf : 102,
                    recordType: 'customrecord_scv_test_order',
                    recordId: 1,
                }
            });

            form.addButton({
                id: 'custpage_btn_populate_sales_ord',
                label : 'Populate Sales Order',
                functionName : "window.open('" + createUrl + "');",
            })
        }

        const addButtonPrintTestOrder = (form, newRecord) => {
            let viewUrl = url.resolveScript({
                scriptId: 'customscript_scv_sl_print_test',
                deploymentId: 'customdeploy_scv_sl_print_test',
            ...
            form.addButton({
                id: 'custpage_bt_print_testorder',
                label: 'Print Test Order',
                functionName: "window.open('" + viewUrl + "');"
            });
        }

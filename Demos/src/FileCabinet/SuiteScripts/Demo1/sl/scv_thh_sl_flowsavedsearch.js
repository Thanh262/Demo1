...
define(['N/ui/serverWidget', 'N/search'], (serverWidget, search) => {
    ...
    const setValueLineOfTestOrderLines = (sublist, parameters, searchResults) => {
        ...
        for (let i = 0; i < searchResults.length; i++) {
            ...
            let grossAmount = parseFloat(result.getValue('custrecord_scv_test_odl_grossamt'));
            let interestAmount = grossAmount * interestRate;
            
            sublist.setSublistValue({
                id: 'custpage_item',
                line: i,
                value: result.getText('custrecord_scv_test_odl_item')
            });
            sublist.setSublistValue({
    ...
    const createForm = (parameters) => {
        ...
        form.clientScriptModulePath = '../cs/scv_thh_cs_flowsavedsearch';
    ...
    const searchTestOrderLines = (parameters) => {
        let orderLineSearch = search.load('customsearch_scv_thh_test_orderline_2');
        let filters = orderLineSearch.filters;
        if (parameters.custpage_customer) {
            filters.push(search.createFilter({name: 'custrecord_scv_test_order_customer', join: 'custrecord_scv_test_odl_order', operator: search.Operator.ANYOF, values: parameters.custpage_customer}));
        }
        
        if (parameters.custpage_subsidiary) {
            filters.push(search.createFilter({name: 'custrecord_scv_tol_itsub', operator: search.Operator.ANYOF, values: parameters.custpage_subsidiary}));
        }

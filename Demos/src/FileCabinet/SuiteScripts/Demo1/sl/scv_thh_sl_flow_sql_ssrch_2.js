...
define(['N/query', 'N/render', 'N/ui/serverWidget', 'N/search', '../olib/alasql.min@1.7.3', '../common/scv_common_pdf'],
    (query, render, serverWidget, search, alasql, comPdf) => {
        const onRequest = (scriptContext) => {
            ...
            if (request.method === 'GET') {
                ...
                if (parameters.isSearch === 'T') {
                    ...
                    let listDataTestOrder = joinSearchAndQuery(listDataTestOrderLines, listItemsAvaiable);
                ...
                if (parameters.isExportPdf === 'T') {
                    let bodyParameter = JSON.parse(request.body);
                    let listDataTestOrderLines = queryListDataTestOrderLines(bodyParameter);
                    //them vao search
                    let listItemsAvaiable = searchListDataItemAvailable();
                    let listDataTestOrder = joinSearchAndQuery(listDataTestOrderLines, listItemsAvaiable);

                    let objFilePdf = buildFilePdf(bodyParameter, listDataTestOrder);
        ...
        const createForm = (parameters) => {
            ...
            form.clientScriptModulePath = '../cs/scv_thh_cs_flow_sql_ssrch';
        ...
        const joinSearchAndQuery =(listDataTestOrderLines, listItemsAvaiable) => {
            return alasql("select a.item, b.itemid, a.grossamt, b.quantityavailable FROM ? as a JOIN ? as b ON a.item = b.item ", [listDataTestOrderLines, listItemsAvaiable]);
        }
